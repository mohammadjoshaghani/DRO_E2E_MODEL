...
class DataSet:
    def __init__(self, n_obs):
        ...
        X = pd.read_pickle("dataset/X_iran.pkl")[
            : -(1 + LG + EH)
        ]  # initial size: 1135*8
        ...
        X = X.unfold(0, LB, 1).permute(
            0, 2, 1
        )  # batch, length, features :(550, 32, 20)
        Y = Y.unfold(0, LB + EH, 1).permute(
            0, 2, 1
        )  # batch, length, features :(550, 45, 20)
