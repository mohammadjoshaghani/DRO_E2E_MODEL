...
class Runner:
    ...
    def _init(self, mode, epochs, lr=0.0125, train_gamma=True, train_delta=True):
        self.mode = mode
        self.epochs = epochs
    ...
    def loss(self, z, y, predLoss):
        ...
        # loss = 0.5/20 * predLoss + 1/len(self.dataLoader) * -sharpe_r
        ...
        if self.epochs != 1:
            try:
                self.model.load_state_dict(
                    torch.load(self.path_model + f"model{self.epochs-1}.pt")
                )
            except:
                raise ValueError(
                    f"there is no trained model with providing hayper-parameters.\n\
    ...
    def _portfolio(self):
        y_true = self.dataLoader.dataset.tensors[1][
            :, self.n_obs : self.n_obs + self.FH, :, self.LB : self.LB + 1
        ].squeeze(3)
    ...
    def save(self):
        ...
        np.savetxt(
            self.path + "/loss_" + str(self.mode) + ".csv", self.L, delimiter=","
        )
        np.savetxt(
            self.path + "/predLoss_" + str(self.mode) + ".csv",
            self.predLoss,
            delimiter=",",
        )
        np.savetxt(
            self.path + "/portReturns_" + str(self.mode) + ".csv",
            self.portfolio_return.detach().cpu().numpy(),
            delimiter=",",
        )
        ...
        if self.mode == "train" and self.model_name != "Equally_weighted":
            torch.save(
                self.model.state_dict(), self.path_model + f"model{self.epochs}.pt"
            )
